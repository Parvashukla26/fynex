...
const Portfolio = () => {
  return (
    <section className='py-32 md:py-64 bg-[#1a1a1a] overflow-hidden' id='portfolio'>
          ...
          {portfolioData.map((item, index) => {
            const CardContent = (
              <motion.div
                ...
                className={`group relative ${item.link ? 'cursor-pointer' : ''}`}
              >
                {/* Image Container */}
                <div className='relative overflow-hidden mb-8 aspect-[16/10] bg-[#252525] border border-white/5 rounded-sm transition-all duration-700 group-hover:border-primary/20'>
                  <Image
                    src={item.image || '/images/portfolio/img-portfolio.png'}
                    alt={item.title}
                    fill
                    className='object-contain p-4 grayscale brightness-75 group-hover:grayscale-0 group-hover:scale-105 group-hover:brightness-100 transition-all duration-1000 ease-out cursor-pointer'
                  />
